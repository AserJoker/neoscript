...
static void neo_init_object(void *target, void *source, void *_) {
  ...
  dst->properties = create_neo_map((neo_compare_fn)cstring_compare, free, free);
}
...
neo_list neo_object_get_keys(neo_context ctx, neo_value value) {
  ...
  while (node != neo_list_tail(keys)) {
    cstring key = neo_list_node_get(node);
    if (key) {
...
void neo_object_set_field(neo_context ctx, neo_value value, const cstring name,
  ...
  if (!attr_impl) {
    ...
    neo_map_set(impl->properties, cstring_clone(name), attr_impl);
  } else {
