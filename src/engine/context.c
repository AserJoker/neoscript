...
typedef struct _neo_task_context *neo_task_context;
struct _neo_task_context {
  neo_scope scope;
  neo_value closure;
  ...
  neo_task_context next;
  neo_task_context last;
};
neo_task_context create_task_context() {
  neo_task_context coroutine =
      (neo_task_context)malloc(sizeof(struct _neo_task_context));
...
void free_task_context(neo_task_context routine) {
  neo_task_context last = routine->last;
  neo_task_context next = routine->next;
  if (last) {
    last->next = next;
  }
  if (next) {
...
struct _neo_context {
  ...
  neo_task_context task_context;

  neo_task_context mainroutine;
...
neo_context create_neo_context(neo_runtime rt) {
  ...
  ctx->rt = rt;
  ctx->task_context = create_task_context();
  ctx->task_context->next = ctx->task_context;
  ctx->task_context->last = ctx->task_context;
  ctx->mainroutine = ctx->task_context;
...
void free_neo_context(neo_context ctx) {
  ...
  free_task_context(ctx->task_context);
  free(ctx);
}
void neo_context_push_call_frame(neo_context self, const char *funcname,
                                 const char *filename, int line, int column) {
  neo_call_frame frame = create_neo_call_frame(funcname);
  if (self->task_context->callstacks) {
    self->task_context->callstacks->column = column;
    self->task_context->callstacks->line = line;
    self->task_context->callstacks->filename = strings_clone(filename);
  }
  frame->parent = self->task_context->callstacks;
  self->task_context->callstacks = frame;
}
void neo_context_pop_call_frame(neo_context self) {
  neo_call_frame frame = self->task_context->callstacks;
  self->task_context->callstacks = frame->parent;
  if (frame->filename) {
    free(frame->filename);
  }
  if (frame->funcname) {
  ...
  free(frame);
  if (self->task_context->callstacks) {
    if (self->task_context->callstacks->filename) {
      free(self->task_context->callstacks->filename);
      self->task_context->callstacks->filename = NULL;
    }
    self->task_context->callstacks->line = 0;
    self->task_context->callstacks->column = 0;
  }
}
neo_runtime neo_context_get_runtime(neo_context self) { return self->rt; }

void neo_context_push_scope(neo_context self) {
  neo_scope scope = create_neo_scope(self->task_context->scope);
  self->task_context->scope = scope;
}
void neo_context_pop_scope(neo_context self) {
  neo_scope parent = neo_scope_get_parent(self->task_context->scope);
  free_neo_scope(self->task_context->scope);
  self->task_context->scope = parent;
}

neo_scope neo_context_get_scope(neo_context self) {
  return self->task_context->scope;
}
neo_value neo_context_create_value(neo_context self, neo_type type,
                                   void *init) {
  neo_atom atom = create_neo_atom(type, init);
  neo_value value = create_neo_value(self->task_context->scope, atom);
  return value;
}
neo_value neo_context_call(neo_context self, neo_value closure, int argc,
  ...
  neo_value last = self->task_context->closure;
  self->task_context->closure = closure;
...
neo_value neo_context_get_closure(neo_context self) {
  return self->task_context->closure;
}

jmp_buf *neo_context_try_start(neo_context self) {
  neo_try_block block = create_neo_try_block(self->task_context->scope);
  neo_list_push(self->task_context->trystacks, block);
  return &block->context;
}

void neo_context_try_end(neo_context self) {
  neo_try_block block = neo_list_pop(self->task_context->trystacks);
  if (block) {
...
void neo_context_throw(neo_context self, neo_value exception) {
  neo_try_block block = (neo_try_block)neo_list_node_get(
      neo_list_node_last(neo_list_tail(self->task_context->trystacks)));
  if (block) {
