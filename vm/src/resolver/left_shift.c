...
neo_value neo_resolver_left_shift(neo_vm vm, neo_ast node) {

  neo_context ctx = neo_vm_get_context(vm);
  if (!node->right || !node->left) {
    neo_context_throw(
        ctx, create_neo_exception(ctx, "Invalid operator left_shift", NULL,
  ...
  char message[1024];
  sprintf(message, "Invalid operator left_shift with type (%d and %d)",
