...
neo_value neo_resolver_mod(neo_vm vm, neo_ast node) {

  neo_context ctx = neo_vm_get_context(vm);
  if (!node->right || !node->left) {
    neo_context_throw(ctx, create_neo_exception(ctx, "Invalid operator mod",
  ...
  char message[1024];
  sprintf(message, "Invalid operator mod with type (%d and %d)",
