...
neo_value neo_resolver_add_and_set(neo_vm vm, neo_ast node) {

  neo_context ctx = neo_vm_get_context(vm);
  if (!node->right || !node->left) {
    neo_context_throw(
        ctx, create_neo_exception(ctx, "Invalid operator add_and_set", NULL,
  ...
  char message[1024];
  sprintf(message, "Invalid operator add_and_set with type (%d and %d)",
